...
/**
 * cnd_http_request($method, $url, $body, $headers, $userpwd)
 * ──────────────────────────────────────────────────────────────
 * Robust HTTP client that works even if the cURL extension is disabled.
 * Uses cURL if available, otherwise falls back to file_get_contents().
 */
if (!function_exists('cnd_http_request')) {
    function cnd_http_request(string $method, string $url, $body = null, array $headers = [], $userpwd = null)
    {
        $method = strtoupper($method);
        $payload = (is_array($body) || is_object($body)) ? json_encode($body) : $body;
        ...
        if (function_exists('curl_init')) {
            ...
            $opts = [
                ...
                CURLOPT_TIMEOUT        => 15,
                CURLOPT_SSL_VERIFYPEER => false, // often needed on dev envs
            ];
            if ($method === 'POST') {
            ...
            return (object)['body' => $result, 'code' => $code, 'error' => $error];
        }

        // 2. Fallback to file_get_contents (stream context)
        if ($userpwd) {
            $headers[] = 'Authorization: Basic ' . base64_encode($userpwd);
        }
        if ($payload && !preg_grep('/^Content-Type:/i', $headers)) {
        ...
        return (object)['body' => $result, 'code' => $code, 'error' => ($result === false ? 'Stream error' : null)];
    }
}
...
/**
 * listing_img_url($path, $placeholder)
 * ──────────────────────────────────────────────────────────────
 * Returns an absolute URL for a listing cover / gallery image.
 * Handles three storage formats that exist in the DB:
 *   (a) just the filename:            "ballet.jpg"
 *   (b) uploads/ prefix only:         "uploads/ballet.jpg"
 *   (c) full relative prefix:         "uploads/listings/ballet.jpg"
 *
 * All three map to: base_url('uploads/listings/ballet.jpg')
 * Prevents the doubled-path bug: uploads/listings/uploads/ballet.jpg
 */
if (!function_exists('listing_img_url')) {
    function listing_img_url(string $path = '', string $placeholder = 'assets/frontend/img/class-placeholder.jpg'): string
    {
        $path = trim($path);
        if ($path === '') {
        ...
        // Strip any leading "uploads/listings/" prefix (stored as full sub-path)
        $path = preg_replace('#^uploads[/\\\\]listings[/\\\\]#i', '', $path);
        // Also strip a bare "uploads/" prefix (old seed data)
        $path = preg_replace('#^uploads[/\\\\]#i', '', $path);

        // Normalise directory separators to forward slashes
        $path = str_replace('\\', '/', $path);

        return base_url('uploads/listings/' . $path);
    }
}
  ...
  if (!function_exists('is_logged')) {
    ...
    function is_logged()
    {
      ...
      if($isLogged){
        ...
        $user = $userModel->getById((int)$isLogged->id);
        // verify login_token
  ...
  */
  if (!function_exists('hasPermissions')) {
    ...
    function hasPermissions($code = '') {
        ...
        $builder->join('permissions p', 'p.id = rp.permission');
        $builder->where('rp.role', logged('role'));
